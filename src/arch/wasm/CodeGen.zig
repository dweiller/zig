...
branches: std.ArrayListInlineUnmanaged(Branch) = .{},
...
locals: std.ArrayListInlineUnmanaged(u8),
...
/// are enabled also.
simd_immediates: std.ArrayListInlineUnmanaged([16]u8) = .{},
...
fn genFunctype(
    ...
    var temp_params = std.ArrayListInline(wasm.Valtype).init(gpa);
    defer temp_params.deinit();
    var returns = std.ArrayListInline(wasm.Valtype).init(gpa);
...
fn genFunc(func: *CodeGen) InnerError!void {
    ...
    if (func.initial_stack_value != .none) {
        var prologue = std.ArrayListInline(Mir.Inst).init(func.gpa);
...
fn resolveCallingConventionValues(func: *CodeGen, fn_ty: Type) InnerError!CallWValues {
    ...
    var args = std.ArrayListInline(WValue).init(func.gpa);
    defer args.deinit();
...
fn airSwitchBr(func: *CodeGen, inst: Air.Inst.Index) InnerError!void {
    ...
    var case_list = try std.ArrayListInline(struct {
...
fn getTagNameFunction(func: *CodeGen, enum_ty: Type) InnerError!u32 {
    ...
    var relocs = std.ArrayListInline(link.File.Wasm.Relocation).init(func.gpa);
    defer relocs.deinit();

    var body_list = std.ArrayListInline(u8).init(func.gpa);
...
fn airErrorSetHasValue(func: *CodeGen, inst: Air.Inst.Index) InnerError!void {
    ...
    var values = try std.ArrayListInline(u32).initCapacity(func.gpa, names.len);
    defer values.deinit();
