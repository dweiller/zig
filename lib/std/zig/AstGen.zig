...
fn expr(gz: *GenZir, scope: *Scope, ri: ResultInfo, node: Ast.Node.Index) InnerError!Zir.Inst.Ref {
    ...
    switch (tree.nodeTag(node)) {
        ...
        => {
            ...
            return builtinCall(gz, scope, ri, node, params, .none, .anon);
        },
        ...
        => {
            ...
            switch (info.tag) {
                .Enum, .Struct, .Union => {
                    var buf: [2]Ast.Node.Index = undefined;
                    const params = tree.builtinCallParams(&buf, node).?;
                    return try builtinCall(gz, scope, ri, node, params, .none, name_strat);
                },
                else => return null,
            }
        },
        ...
        while (true) {
            switch (tree.nodeTag(inner_node)) {
                ...
                => {
                    ...
                    const result = try builtinCall(gz, scope, .{ .rl = .none }, inner_node, params, allow_hint, .anon);
